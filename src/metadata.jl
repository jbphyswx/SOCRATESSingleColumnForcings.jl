...
function get_Tg_offset(flight_number::Integer, ::Type{FT} = Float32, ::Val{use_summary_nc} = Val(false)) ::FT where {FT, use_summary_nc}
    if use_summary_nc
        return NC.Dataset(atlas_socrates_summary_file(flight_number), "r") do summary
            flight_ind = findfirst(vec(Array(summary["flight_number"])) .== flight_number)
            FT(summary[:deltaT][flight_ind])
        end
    else
...
function get_socrates_initial_time(flight_number::Integer, ::Val{use_summary_nc} = Val(false)) where {use_summary_nc}
    if use_summary_nc
        return NC.Dataset(atlas_socrates_summary_file(flight_number), "r") do summary
            flight_ind = findfirst(vec(Array(summary["flight_number"])) .== flight_number)
            summary["reference_time"][flight_ind] - Dates.Hour(12)
        end
    else
        if flight_number == 1
