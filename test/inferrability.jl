...
Test.@testset "hot-path inferrability" begin
    ...
    su = I.build_spline(I.FastLinear1DInterpolation, xu, fu; bc = ext, drop_collinear = false)
    xi = [0.0, 1.0, 3.0, 7.0, 15.0, 31.0]; fi = @. cos(xi / 4)   # irregular grid → IrregularNodes
    si = I.build_spline(I.FastLinear1DInterpolation, xi, fi; bc = ext, drop_collinear = false)
